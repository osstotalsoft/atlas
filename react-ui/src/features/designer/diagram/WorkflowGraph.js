...
const WorkflowGraph = ({ style, className, dag, executionMode, onClick }) => {
  const renderer = new dagreD3Render()
  renderer.shapes().bar = barRenderer
  renderer.shapes().stack = stackRenderer
  ...
  const collapseDfChildren = (parentRef, childrenRef) => {
    ...
    for (const childRef of childrenRef) {
      graph.successors(childRef).forEach(successorRef => tailSet.add(successorRef))
    ...
    graph.setEdge(placeholderRef, successorRef, successor.status ? { executed: true } : undefined)
  }
  ...
  const drawGraph = () => {
    ...
    const dfNodes = dagGraph.nodes().filter(nodeId => dagGraph.node(nodeId).type === 'FORK_JOIN_DYNAMIC')

    for (const parentRef of dfNodes) {
      const childRefs = dagGraph.successors(parentRef)

      if (childRefs.length > 2) {
        collapseDfChildren(parentRef, childRefs)
      }
    }

    // Collapse Do_while children
    const doWhileNodes = dagGraph.nodes().filter(nodeId => dagGraph.node(nodeId).type === 'DO_WHILE')
  ...
  const handleClick = e => {
    ...
    } else if (node.type === 'DF_EMPTY_PLACEHOLDER' || node.type === 'TERMINAL') {
  ...
  const renderVertex = nodeId => {
    const dagGraph = dag.graph
    const graphx = graph

    const v = dagGraph.node(nodeId) || graphx.node(nodeId) // synthetic nodes (e.g. DF placeholder) not found in 'dag' but preloaded into local graph.
    ...
    switch (v.type) {
      ...
      case 'DF_EMPTY_PLACEHOLDER':
        retval.label = v.status ? 'No tasks spawned' : 'Dynamically spawned tasks'
        retval.shape = 'stack'
        break
  ...
  const expandBar = barRef => {
    ...
    translateX = minX
    barNode.elem.setAttribute('transform', `translate(${translateX}, ${translateY})`)
    ...
    text.setAttribute('transform', `translate(${newTextTransformX}, ${currTextTransformY})`)
  }
    ...
    <div style={style} className={`graphWrapper ${className || ''}`}>
      <svg ref={svgRef} className='graphSvg'>
        <defs>
          ...
          <filter id='dropShadow' height='300%' width='300%' x='-75%' y='-75%'>
          ...
          <marker id='endarrow' markerWidth='8' markerHeight='6' refX='8' refY='3' orient='auto' markerUnits='strokeWidth'>
            <polygon points='0 0, 8 3, 0 6' />
          </marker>

          <marker id='startarrow' markerWidth='8' markerHeight='6' refX='0' refY='3' orient='auto' markerUnits='strokeWidth'>
...
function barRenderer(parent, bbox, node) {
  ...
  node.intersect = function (point) {
    ...
    return {
      x:
        (node.fanDir === 'down' && point.y > node.y) || (node.fanDir === 'up' && point.y < node.y)
          ? point.x
...
function stackRenderer(parent, bbox, node) {
  ...
  group
    ...
    .attr('width', bbox.width)
    .attr('height', bbox.height)
    .attr('transform', `translate(${-bbox.width / 2 - STACK_OFFSET * 2}, ${-bbox.height / 2 - STACK_OFFSET * 2})`)
  group
    .insert('rect')
    .attr('width', bbox.width)
    .attr('height', bbox.height)
    .attr('transform', `translate(${-bbox.width / 2 - STACK_OFFSET}, ${-bbox.height / 2 - STACK_OFFSET})`)
  group
    .insert('rect')
    .attr('width', bbox.width)
    .attr('height', bbox.height)
