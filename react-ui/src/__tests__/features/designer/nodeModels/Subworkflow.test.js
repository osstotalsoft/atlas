...
import SubworkflowNodeModel from 'features/designer/nodeModels/subworkflowNode/SubworkflowNodeModel'
...
describe('SUBWORKFLOW', () => {
  it('Should validate if the node has all ports connected with the corresponding number of links', () => {
    ...
    const subworkflow = new SubworkflowNodeModel()
    const subworkflow_in = subworkflow.addInPort('in')
    const subworkflow_out = subworkflow.addInPort('out')

    //input port not connected
    model.addAll(subworkflow)
    expect(subworkflow.validate()[1]).toContain(errorMessages.notLinked)

    const start = new StartNodeModel()
    const start_out = start.addPort(new DefaultPortModel({ in: false, name: 'out', type: 'START' }))

    const link1 = start_out.link(subworkflow_in)
    ...
    const link2 = subworkflow_out.link(lambda1_in)

    //node connected correctly
    model.addAll(link2)
    expect(subworkflow.validate()[0]).toBe(true)

    const lambda2 = new LambdaNodeModel()
    const lambda2_in = lambda2.addInPort('in')
    const link3 = subworkflow_out.link(lambda2_in)

    //node has two output links
    model.addAll(link3)
    expect(subworkflow.validate()[1]).toContain(errorMessages.multipleLinks)
  })
})
