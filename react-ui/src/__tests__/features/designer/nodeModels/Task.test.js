...
import TaskNodeModel from 'features/designer/nodeModels/taskNode/TaskNodeModel'
...
describe('TASK', () => {
  it('Should validate if the node has all ports connected with the corresponding number of links', () => {
    ...
    const task = new TaskNodeModel()
    const task_in = task.addInPort('in')
    const task_out = task.addInPort('out')

    //input port not connected
    model.addAll(task)
    expect(task.validate()[1]).toContain(errorMessages.notLinked)

    const start = new StartNodeModel()
    const start_out = start.addPort(new DefaultPortModel({ in: false, name: 'out', type: 'START' }))

    const link1 = start_out.link(task_in)
    ...
    const link2 = task_out.link(lambda1_in)

    //node connected correctly
    model.addAll(link2)
    expect(task.validate()[0]).toBe(true)

    const lambda2 = new LambdaNodeModel()
    const lambda2_in = lambda2.addInPort('in')
    const link3 = task_out.link(lambda2_in)

    //node has two output links
    model.addAll(link3)
    expect(task.validate()[1]).toContain(errorMessages.multipleLinks)
  })
})
